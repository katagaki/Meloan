    ...
    func createPDF() -> Data {
        ...
        let pdfData = renderer.pdfData { context in
            ...
            let titleSize = drawTitle(receipt.name,
                                      x: minX, y: minY)
            // Draw logo
            drawImage(UIImage(named: "PDF.Watermark")!,
                      x: 597.6 - 10.0 - titleSize.height, y: 10.0,
            ...
            for item in receipt.items() {
                let itemNameSize = drawLeadingText(item.name,
                                                   x: minX, y: minY)
                let priceSize = drawTrailingText(String(format: "%.2f", item.price),
                                                 font: .monospacedSystemFont(ofSize: 16.0, weight: .regular),
                                                 x: minX, y: minY)
                minY += max(itemNameSize.height, priceSize.height) + 10.0
                if let person = item.person {
                    let personNameSize = drawLeadingSecondaryText(person.name,
                                                                  x: minX + 26.0, y: minY)
                    if let photo = person.photo {
                        drawImage(UIImage(data: photo)!,
            ...
            let totalBeforeTaxSize = drawLeadingText(NSLocalizedString("Receipt.Total.BeforeTax", comment: ""),
                                                     x: minX, y: minY)
            let totalBeforeTaxPriceSize = drawTrailingText(String(format: "%.2f", receipt.sumOfItems()),
                                                           font: .monospacedSystemFont(ofSize: 16.0, weight: .regular),
                                                           x: minX, y: minY)
            ...
            let totalAfterTaxSize = drawLeadingText(NSLocalizedString("Receipt.Total.AfterTax", comment: ""),
                                                    x: minX, y: minY)
            let totalAfterTaxPriceSize = drawTrailingText(String(format: "%.2f", receipt.sum()),
                                                          font: .monospacedSystemFont(ofSize: 16.0, weight: .regular),
                                                          x: minX, y: minY)
